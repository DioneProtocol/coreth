...
//
// CreateAccount is called during the DELTA CREATE operation. The situation might arise that
...
// used when the DELTA emits new state logs. It should be invoked before
...
func (s *StateDB) Prepare(rules params.Rules, sender, coinbase common.Address, dst *common.Address, precompiles []common.Address, list types.AccessList) {
	if rules.IsApricotPhase2 {
		...
		if dst != nil {
			al.AddAddress(*dst)
			// If it's a create-tx, the destination will be added inside delta.create
		}
