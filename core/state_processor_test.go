...
func TestStateProcessorErrors(t *testing.T) {
	...
	{ // Tests against a 'recent' chain definition
		...
		for i, tt := range []struct {
			...
			{ // ErrNonceTooLow
				txs: []*types.Transaction{
					makeTx(key1, 0, common.Address{}, big.NewInt(0), params.TxGas, big.NewInt(238095238095238), nil),
					makeTx(key1, 0, common.Address{}, big.NewInt(0), params.TxGas, big.NewInt(238095238095238), nil),
				},
			...
			{ // ErrNonceTooHigh
				txs: []*types.Transaction{
					makeTx(key1, 100, common.Address{}, big.NewInt(0), params.TxGas, big.NewInt(238095238095238), nil),
				},
			...
			{ // ErrGasLimitReached
				txs: []*types.Transaction{
					makeTx(key1, 0, common.Address{}, big.NewInt(0), 15000001, big.NewInt(238095238095238), nil),
				},
				want: "could not apply tx 0 [0x6a344abc58d6f44eca47d2441abcab97fcc66e80445ba3677c822677ddf42dc2]: gas limit reached",
			},
			{ // ErrInsufficientFundsForTransfer
				txs: []*types.Transaction{
					makeTx(key1, 0, common.Address{}, new(big.Int).Mul(big.NewInt(4000000000000000000), big.NewInt(10000)), params.TxGas, big.NewInt(238095238095238), nil),
				},
			...
			{ // ErrInsufficientFunds
				txs: []*types.Transaction{
					makeTx(key1, 0, common.Address{}, big.NewInt(0), params.TxGas, new(big.Int).Mul(big.NewInt(900000000000000000), big.NewInt(10000)), nil),
				},
			...
			{ // ErrIntrinsicGas
				txs: []*types.Transaction{
					makeTx(key1, 0, common.Address{}, big.NewInt(0), params.TxGas-1000, big.NewInt(238095238095238), nil),
				},
			...
			{ // ErrGasLimitReached
				txs: []*types.Transaction{
					makeTx(key1, 0, common.Address{}, big.NewInt(0), params.TxGas*762, big.NewInt(238095238095238), nil),
				},
			...
			{ // ErrBlobFeeCapTooLow
				...
				want: "could not apply tx 0 [0x6c11015985ce82db691d7b2d017acda296db88b811c3c60dc71449c76256c716]: max fee per gas less than block base fee: address 0x71562b71999873DB5b286dF957af199Ec94617F7, maxFeePerGas: 1 baseFee: 238095238095238",
			},
		} {
