...
RUN git clone -b $DIONE_VERSION --single-branch https://github.com/dioneprotocol/dionego.git

# Copy coreth repo into desired location
COPY . coreth

# Set the workdir to DioneGo and update coreth dependency to local version
WORKDIR $GOPATH/src/github.com/dioneprotocol/dionego
# Run go mod download here to improve caching of DioneGo specific depednencies
RUN go mod download
...
# Build the DioneGo binary with local version of coreth.
...
RUN mkdir -p /dionego/build
WORKDIR /dionego/build

# Copy the executables into the container
COPY --from=builder /go/src/github.com/dioneprotocol/dionego/build .
