...
type Client interface {
	...
	ExportDIONE(ctx context.Context, userPass api.UserPass, amount uint64, to string) (ids.ID, error)
...
func (c *client) ListAddresses(ctx context.Context, user api.UserPass) ([]string, error) {
	...
	err := c.requester.SendRequest(ctx, "dione.listAddresses", &user, res)
...
func (c *client) ExportDIONE(
	...
	return c.Export(ctx, user, amount, to, "DIONE")
}
