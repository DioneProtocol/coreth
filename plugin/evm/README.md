...
The EVM package implements the OdysseyGo VM interface.
...
Atomic transactions utilize Shared Memory (documented [here](https://github.com/DioneProtocol/odysseygo/blob/master/chains/atomic/README.md)) to send assets to the P-Chain and A-Chain.
...
The C-Chain uses the account model for its own state, but atomic transactions must be compatible with the P-Chain and A-Chain, such that C-Chain atomic transactions must transform between the account model and the UTXO model.
