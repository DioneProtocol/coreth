...
type atomicTxTest struct {
	...
	// respective step.
	semanticVerifyErr, deltaStateTransferErr, acceptErr string
...
func executeTxTest(t *testing.T, test atomicTxTest) {
	...
	// Retrieve dummy state to test that DELTAStateTransfer works correctly
	sdb, err := vm.blockChain.StateAt(lastAcceptedBlock.ethBlock.Root())
	if err != nil {
		t.Fatal(err)
	}
	if err := tx.UnsignedAtomicTx.DELTAStateTransfer(vm.ctx, sdb); len(test.deltaStateTransferErr) == 0 && err != nil {
		t.Fatalf("DELTAStateTransfer failed unexpectedly due to: %s", err)
	} else if len(test.deltaStateTransferErr) != 0 {
		if err == nil {
			t.Fatalf("DELTAStateTransfer unexpectedly returned a nil error. Expected err: %s", test.deltaStateTransferErr)
		}
		if !strings.Contains(err.Error(), test.deltaStateTransferErr) {
			t.Fatalf("Expected SemanticVerify to fail due to %s, but failed with: %s", test.deltaStateTransferErr, err)
		}
		// If DELTAStateTransfer failed for the expected reason, return early
		return
	}
