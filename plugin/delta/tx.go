...
var (
	TxBytesGas     uint64 = 1
	DELTAOutputGas uint64 = (common.AddressLength + wrappers.LongLen + hashing.HashLen) * TxBytesGas
	DELTAInputGas  uint64 = (common.AddressLength+wrappers.LongLen+hashing.HashLen+wrappers.LongLen)*TxBytesGas + secp256k1fx.CostPerSignature
)

// DELTAOutput defines an output that is added to the DELTA state created by import transactions
...
// Verify ...
func (out *DELTAOutput) Verify() error {
	switch {
	case out == nil:
		return errNilOutput
	case out.Amount == 0:
...
// Verify ...
func (in *DELTAInput) Verify() error {
	switch {
	case in == nil:
		return errNilInput
	case in.Amount == 0:
...
type UnsignedAtomicTx interface {
	...
	DELTAStateTransfer(ctx *snow.Context, state *state.StateDB) error
}
...
// innerSortInputsAndSigners implements sort.Interface for DELTAInput
...
// SortDELTAInputsAndSigners sorts the list of DELTAInputs based on the addresses and assetIDs
func SortDELTAInputsAndSigners(inputs []DELTAInput, signers [][]*secp256k1.PrivateKey) {
