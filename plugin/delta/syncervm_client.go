...
type StateSyncClient interface {
	...
	// additional methods required by the delta package
...
// stateSync blockingly performs the state sync for the DELTA state and the atomic state
...
func (client *stateSyncerClient) stateSync(ctx context.Context) error {
	...
	// Sync the DELTA trie and then the atomic trie. These steps could be done
...
func (client *stateSyncerClient) syncStateTrie(ctx context.Context) error {
	...
	deltaSyncer, err := statesync.NewStateSyncer(&statesync.StateSyncerConfig{
	...
	if err := deltaSyncer.Start(ctx); err != nil {
		return err
	}
	err = <-deltaSyncer.Done()
...
func (client *stateSyncerClient) finishSync() error {
	...
	deltaBlock, ok := wrapper.Block.(*Block)
	if !ok {
		return fmt.Errorf("could not convert block(%T) to delta.Block", stateBlock)
	}

	deltaBlock.SetStatus(choices.Accepted)
	block := deltaBlock.ethBlock
	...
	if err := client.state.SetLastAcceptedBlock(deltaBlock); err != nil {
		return err
	}
